...
void loop()
{
  ...
  for (int sensor = 0; sensor < numSensor; sensor++)  
    ...
    readings[5][index] = cs_4_10.capacitiveSensor(30);   // read from the sensor
    readings[6][index] = cs_4_11.capacitiveSensor(30);   // read from the sensor
    //readings[7][index] = cs_4_14.capacitiveSensor(30);   // read from the sensor
  for (int sensor = 0; sensor < numSensor; sensor++) {  
  ...
  Serial.print(tau_filtered[0]);                // print sensor output
    ...
    Serial.print(tau_filtered[4]);                // print sensor output
    Serial.print('\t');
    Serial.print(tau_filtered[5]);
    Serial.print('\t');
    Serial.println(tau_filtered[6]);
    //Serial.print('\t');
  ...
  if (status == ACI_EVT_CONNECTED) {
    ...
    */  
    ...
    if (tau_filtered[0] > 300)
      touch1 = true;
    if (touch1 == true && tau_filtered[0] < 200){  
      int value = 1;
