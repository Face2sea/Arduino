...
long total[4];                  // the running total of # readings
long tau_filtered[4];                // the running average signal -- similar with low-pass filtered signal 
float longterm_mean[4];          // long term mean for detrending
float longterm_sd[4];            // long term s.d. of tau_filtered.
long tau_anomaly[4];                    // final signal for thresholding to generate orders
...
void loop()
{
  ...
  if (status == ACI_EVT_CONNECTED) {
    ...
    if (tau_anomaly[0] > long(SNR_touch*longterm_sd[0]+offset_touch))
      touch1 = true;
    if (touch1 == true && tau_anomaly[0] < long(SNR_touch*longterm_sd[0]+offset_touch)){  
      int value = 1;
    ...
    if (tau_anomaly[1] > long(SNR_appro*longterm_sd[1]+offset_appro)){
      handon2 = true;
    }
    if (handon2 == true && tau_anomaly[1] < long(SNR_appro*longterm_sd[1]+offset_appro)){
        int value2 = 10;
    ...
    if (tau_anomaly[2] > long(SNR_appro*longterm_sd[2]+offset_appro)){
      handon3 = true;
    }   
    if (handon3 == true && tau_anomaly[2] < long(SNR_appro*longterm_sd[2]+offset_appro)){
        int value3 = 11;
    ...
    if (tau_anomaly[3] > long(SNR_appro*longterm_sd[3]+offset_appro)){
      handon4 = true;
    }   
    if (handon4 == true && tau_anomaly[3] < long(SNR_appro*longterm_sd[3]+offset_appro)){
        int value4 = 12;
